...
export class AppSidebarComponent {
  ...
  private applyRoleBasedSidebar(): void {
    // Find "Dashboard" group (index 0 in your navItems)
    const dashboard = this.navItems.find(n => n.name === 'Dashboard');
    if (!dashboard || !dashboard.subItems) return;

    // Remove any previous injected dialysis items (avoid duplicates)
    dashboard.subItems = dashboard.subItems.filter(s =>
        ![
    ...
    // Insert My Schedule after Dialysis Management
    const dialysisIndex = dashboard.subItems.findIndex(s => s.path === '/dialysis/treatments');

    const inserts: { name: string; path: string }[] = [];


    if (dialysisIndex >= 0) {
      dashboard.subItems.splice(dialysisIndex + 1, 0, ...inserts);
    } else {
      // fallback: append if dialysis item not found
      dashboard.subItems.push(...inserts);
    }
